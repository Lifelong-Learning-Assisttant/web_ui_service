...
class ReactIntegrationTester:
    ...
    async def test_multiple_sessions(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                messages = response.json().get("messages", [])
                # Проверяем что в истории session_1 есть наши сообщения
                has_session1_messages = len(messages) > 0
                print(f"   ✅ Изоляция сессий: {'✅' if has_session1_messages else '❌'}")
                return has_session1_messages
